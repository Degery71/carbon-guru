export const items = [
  {
    src: "/compartir/maduro.jpeg",
    alt: "PLÁTANO MADURO CON QUESO/ RIPE BANANA WITH CHEESE",
    item: "PLÁTANO MADURO CON QUESO/ RIPE BANANA WITH CHEESE",
    price: "4,50€",
    description:
      "Delicioso plátano maduro frito con queso/ delicius fried ripe platain with cheese.",
  },
  {
    src: "/compartir/perrito.jpeg",
    alt: "PERRITO ESPECIAL BRASERO/ BRAZIER SPECIAL DOG",
    item: "PERRITO ESPECIAL BRASERO/ BRAZIER SPECIAL DOG",
    price: "5,50€",
    description:
      "Nuestro perrito especial con salchicha a la brasa, acompañado de baicon, queso y papitas paja, añadiendo salsas especiales a escoger/ our special dog with grilled sausage, accompanied by bacon, cheese and Straw chips, adding special sauces.",
  },
  {
    src: "/compartir/salchipapa.jpeg",
    alt: "SALCHIPAPA",
    item: "SALCHIPAPA",
    price: "6,50€",
    description:
      "Papas fritas acompañadas con salchichas en rodajas, añadiendo diferentes salsas especiales a elección/ frech fries accompanied by sliced sausages adding different special sauces.",
  },

  {
    src: "/compartir/alitas.jpeg",
    alt: "ALITAS DE POLLO/ CHIKEN WINGS",
    item: "ALITAS DE POLLO/ CHIKEN WINGS",
    price: "6,50€",
    description:
      "Alitas de pollo marinadas a la brasa/ grilled marinated chicken wings.",
  },
  {
    src: "/compartir/choripan.jpeg",
    alt: "CHORIPÁN",
    item: "CHORIPÁN",
    price: "6,00€",
    description:
      "Chorizo a la Parrilla con un toque de bacon crujiente y nuestro chimichurri casero, añadiendo salsa de piña y chips de plátano triturados/ Grilled chorizo with a touch of crispy bacon and our homemade chimichurri, adding pineapple sauce and crushed plantain chips.",
  },
  {
    src: "/compartir/choripapa.jpeg",
    alt: "CHORIPAPA",
    item: "CHORIPAPA",
    price: "7,00€",
    description:
      "Papas fritas acompañadas con chorizos en rodajas, añadiendo diferentes salsas especiales a elección/ frech fries accompanied by sliced sausages adding different special sauces.",
  },

  {
    src: "/compartir/morcilla.jpeg",
    alt: "MORCILLA/ BLACK PUDDING",
    item: "MORCILLA/ BLACK PUDDING",
    price: "7,00€",
    description:
      "Morcilla a la brasa acompañada de nuestra salsa especial y papas cocidas/ grilled blood sausage accompanied by our special sauce and bolied potatoes.",
  },
  {
    src: "/compartir/ceviche.jpeg",
    alt: "CEVICHE DE CHICHARRÓN",
    item: "CEVICHE DE CHICHARRÓN",
    price: "9,00€",
    description:
      "Panceta de cerdo marinada en zumo de limón y cocidas a la brasa, añadiendo tomate, cilantro y cebolla morada con nuestra salsa especial/ prok belly marinated in lemon juice and grilled, adding tomato, coriander and onion with our special.",
  },
  {
    src: "/compartir/coctel.jpeg",
    alt: "Cóctel de camarón/ Shrimp cocktail",
    item: "CÓCTEL DE CAMARÓN/ SHRIMP COCKTAIL",
    price: "9,50€",
    description:
      "Langostinos marinado con una cremosa fusión de ketchup y mayonesas, añadiendole cebolla morada, tomate fresco, cilantro, mango y como toque especial, patacones crujientes/ Prawns marinated with a creamy fusion of ketchup and mayonnaise, adding red onion, fresh tomato, coriander, mango and as a special touch, crispy patacones.",
  },
  {
    src: "/compartir/encocado.jpeg",
    alt: "Encocado",
    item: "ENCOCADO DE CAMARÓN/ PRAWN ENCOCADO",
    price: "10,00€",
    description:
      "Langostinos frescos salteados con cebolla morada, pimiento y ajo en una suave salsa de leche de coco con cilantro fresco acompañado de crukientes / Fresh prawns sautéed with red onion, pepper and garlic in a smooth coconut milk sauce with fresh coriander.",
  },
  // {
  //   src: "/compartir/placeholder.png",
  //   alt: "JALEA MIXTA/ MIXED FRIED SEAFOOD",
  //   item: "JALEA MIXTA/ MIXED FRIED SEAFOOD",
  //   price: "10,00€",
  //   description:
  //     "alea mixta, una selección de mariscos frescos fritos a la perfección, crujientes por fuera y tiernos por dentro, acompañados de salsa tártara casera, rodajas de limón fresco y acompañada de yuca/ Mixed fried seafood, a selection of fresh seafood fried to perfection, crispy on the outside and tender on the inside, served with homemade tartar sauce, fresh lemon slices and accompanied by yucca.",
  // },
];
